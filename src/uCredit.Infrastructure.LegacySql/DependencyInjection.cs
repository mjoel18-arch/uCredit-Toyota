...
public static class DependencyInjection
{
    ...
    {
        services.AddOptions<LegacySqlOptions>().Bind(configuration.GetSection(LegacySqlOptions.SectionName)).Validate(options => options.CommandTimeoutSeconds is > 0 and <= 300, "Command timeout must be between 1 and 300 seconds.");
        ...
        services.AddScoped<ICustomerWriteRepository, LegacyCustomerWriteRepository>();
        return services;
    }
}
