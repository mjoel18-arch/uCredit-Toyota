...
export function CustomerCreateView({ onBack, onCreated }: { onBack: () => void; onCreated: (personId: number, pepValidationStatus: 'Executed' | 'NotExecuted') => void }) {
  ...
  return <section className="hero-card customer-create" aria-labelledby="customer-create-title">
    ...
    <form className="customer-create-form" onSubmit={(event) => void submit(event)}>
      ...
      <label>Tipo de domicilio<select value={form.addressTypeCode} onChange={e => set('addressTypeCode', Number(e.target.value))}><option value="1">Dirección única</option><option value="2">Dirección fiscal</option><option value="3">Dirección administrativa</option><option value="4">Dirección social</option></select></label>
      <label>Código postal<input required value={form.postalCode} onChange={e => set('postalCode', e.target.value)} /></label><label>Estado<input required value={form.state} onChange={e => set('state', e.target.value)} /></label><label>Ciudad<input required value={form.city} onChange={e => set('city', e.target.value)} /></label><label>Municipio<input required value={form.municipality} onChange={e => set('municipality', e.target.value)} /></label><label>Colonia<input required value={form.neighborhood} onChange={e => set('neighborhood', e.target.value)} /></label><label>Calle y número<input required value={form.streetAndNumber} onChange={e => set('streetAndNumber', e.target.value)} /></label><label>Número exterior<input required value={form.exteriorNumber} onChange={e => set('exteriorNumber', e.target.value)} /></label>
