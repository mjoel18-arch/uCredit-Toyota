...
import { getCustomerByPersonId, getCustomerProfileReadiness, searchCustomers, type CustomerDetail, type CustomerListItem, type CustomerProfileReadiness } from '../auth/authApi'
...
export function CustomersView({ productName, customerName, onUnauthorized, canCreate = false, onCreated = () => undefined }: { productName: string; customerName: string; onUnauthorized: () => void; canCreate?: boolean; onCreated?: (personId: number, pepValidationStatus?: string) => void }) {
  ...
  async function openDetail(item: CustomerListItem) {
    try {
      const [customer, profileReadiness] = await Promise.all([
        getCustomerByPersonId(item.personId),
        getCustomerProfileReadiness(item.personId),
      ])
      setDetail(customer); setReadiness(profileReadiness); setMessage('')
    }
  ...
  return <section className="hero-card customer-search" aria-labelledby="customers-title">
    ...
    {detail && readiness && <CustomerDetailCard detail={detail} readiness={readiness} />}
...
function CustomerDetailCard({ detail, readiness }: { detail: CustomerDetail; readiness: CustomerProfileReadiness }) {
  const requirements = [
