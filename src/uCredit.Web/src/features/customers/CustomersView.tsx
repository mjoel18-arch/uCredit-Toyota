...
import { activateCustomerAccount, activateCustomerAddress, activateCustomerEmail, activateCustomerPhone, createCustomerAccount, createCustomerAddress, createCustomerEmail, createCustomerPhone, deactivateCustomerAccount, deactivateCustomerAddress, deactivateCustomerEmail, deactivateCustomerPhone, getCustomerAccounts, getCustomerAddresses, getCustomerBanks, getCustomerByPersonId, getCustomerEmailUsages, getCustomerEmails, getCustomerGeneral, getCustomerPhones, getCustomerProfileReadiness, searchCustomers, updateCustomerAccount, updateCustomerAddress, updateCustomerEmail, updateCustomerGeneral, updateCustomerPhone, type CustomerAccountPayload, type CustomerAddressPayload, type CustomerDetail, type CustomerEmailPayload, type CustomerEmailUsage, type CustomerGeneralPayload, type CustomerGeneralProfile, type CustomerListItem, type CustomerPhonePayload, type CustomerProfileReadiness, type ManagedCustomerAccount, type ManagedCustomerAddress, type ManagedCustomerEmail, type ManagedCustomerPhone, type CustomerBank } from '../auth/authApi'
...
function CustomerDetailCard({ detail, readiness, addresses, phones, accounts, emails, canWrite, onUnauthorized, onRefresh }: { detail: CustomerDetail; readiness: CustomerProfileReadiness; addresses: ManagedCustomerAddress[]; phones: ManagedCustomerPhone[]; accounts: ManagedCustomerAccount[]; emails: ManagedCustomerEmail[]; canWrite: boolean; onUnauthorized: () => void; onRefresh: () => Promise<void> }) {
  ...
  return <article className="contract-detail" aria-labelledby="customer-detail-title"><header className="contract-detail-header"><div><span className="eyebrow">Cliente</span><h3 id="customer-detail-title">{detail.name}</h3></div><span className="detail-match">ID {detail.personId}</span></header><dl className="detail-grid"><Detail label="RFC" value={detail.rfc} /><Detail label="Personalidad" value={detail.legalPersonality.description} /><Detail label="Estatus" value={detail.status.description} /><Detail label="Teléfono principal" value={detail.primaryPhone ? [detail.primaryPhone.areaCode, detail.primaryPhone.phoneNumber].filter(Boolean).join(' ') : null} /><Detail label="Correo" value={detail.activeEmails[0]?.email} /><Detail label="Domicilio" value={detail.primaryAddress ? [detail.primaryAddress.streetAndNumber, detail.primaryAddress.exteriorNumber, detail.primaryAddress.city, detail.primaryAddress.state].filter(Boolean).join(', ') : null} /></dl><h4>Requisitos para contrato</h4><ul className="readiness-requirements">{requirements.map(([label, complete, key]) => <li key={key} data-status={complete ? 'complete' : 'pending'}><span>{label}</span><strong>{complete ? 'Completo' : 'Pendiente'}</strong></li>)}</ul><p role="status">{readiness.canCreateContract ? 'Cliente habilitado para contratos.' : 'Este cliente todavía no puede tener contratos.'}</p><GeneralManagement detail={detail} canWrite={canWrite} onUnauthorized={onUnauthorized} onRefresh={onRefresh} /><AddressManagement addresses={addresses} personId={detail.personId} canWrite={canWrite} onUnauthorized={onUnauthorized} onRefresh={onRefresh} /><PhoneManagement phones={phones} personId={detail.personId} canWrite={canWrite} onUnauthorized={onUnauthorized} onRefresh={onRefresh} /><AccountManagement accounts={accounts} personId={detail.personId} canWrite={canWrite} onUnauthorized={onUnauthorized} onRefresh={onRefresh} /><EmailManagement emails={emails} personId={detail.personId} canWrite={canWrite} onUnauthorized={onUnauthorized} onRefresh={onRefresh} /></article>
}

function GeneralManagement({ detail, canWrite, onUnauthorized, onRefresh }: { detail: CustomerDetail; canWrite: boolean; onUnauthorized: () => void; onRefresh: () => Promise<void> }) {
  const [profile, setProfile] = useState<CustomerGeneralProfile | null>(null)
  const [editing, setEditing] = useState(false)
  const [busy, setBusy] = useState(false)
  const [message, setMessage] = useState('')
  const [pepRequired, setPepRequired] = useState(false)
  const [pepConfirmed, setPepConfirmed] = useState(false)
  const [form, setForm] = useState<CustomerGeneralPayload>({ expectedPersonModifiedAt: '', expectedSubtypeModifiedAt: '' })

  async function startEdit() {
    setMessage('')
    try {
      const loaded = await getCustomerGeneral(detail.personId)
      setProfile(loaded)
      setForm({ firstName: loaded.firstName ?? '', paternalSurname: loaded.paternalSurname ?? '', maternalSurname: loaded.maternalSurname ?? '', birthDate: loaded.birthDate?.slice(0, 10), legalName: loaded.legalName ?? '', contactName: loaded.contactName ?? '', contactPosition: loaded.contactPosition ?? '', roleCodes: loaded.roles.filter(role => role.isActive).map(role => role.roleCode), expectedPersonModifiedAt: loaded.personModifiedAt, expectedSubtypeModifiedAt: loaded.subtypeModifiedAt, pepConfirmed: false })
      setEditing(true)
    } catch (error) { if (error instanceof ApiError && error.status === 401) onUnauthorized(); else setMessage(apiErrorMessage(error, 'No fue posible cargar los datos generales.')) }
  }
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (busy || !profile) return
    const roleCodes = form.roleCodes ?? []
    if (roleCodes.length === 0) { setMessage('La persona debe conservar al menos un rol activo.'); return }
    if (profile.roles.some(role => role.roleCode === 10 && role.isActive) && !roleCodes.includes(10)) { setMessage('El rol Aseguradora no puede retirarse.'); return }
    if (profile.roles.some(role => role.isActive && role.roleCode !== 10 && !roleCodes.includes(role.roleCode)) && !window.confirm('¿Confirmas retirar los roles seleccionados?')) return
    setBusy(true); setMessage('')
    try { await updateCustomerGeneral(detail.personId, { ...form, pepConfirmed }); setEditing(false); setProfile(null); await onRefresh(); setMessage('Datos generales guardados correctamente.') }
    catch (error) { if (error instanceof ApiError && error.status === 401) onUnauthorized(); else if (error instanceof ApiError && error.status === 422 && error.code === 'pep_confirmation_required') { setPepRequired(true); setMessage('Confirma la revisión PEP para continuar.') } else if (error instanceof ApiError && error.status === 409 && error.code === 'customer_modified') setMessage('Los datos cambiaron; vuelve a cargar antes de guardar.'); else setMessage(apiErrorMessage(error, 'No fue posible guardar los datos generales.')) }
    finally { setBusy(false) }
  }
  const activeProfile = profile ?? { legalPersonalityCode: detail.legalPersonality.code, rfc: detail.rfc, statusCode: detail.status.code, roles: detail.roles.map(role => ({ roleCode: role.code, roleName: role.description, isActive: true, isEditable: role.code !== 10 })) } as CustomerGeneralProfile
  return <section className="customer-general" aria-labelledby="general-title"><div className="section-heading"><div><span className="eyebrow">Expediente</span><h4 id="general-title">Datos generales</h4></div>{canWrite && <button type="button" onClick={() => void startEdit()} disabled={busy}>Editar</button>}</div><dl className="detail-grid"><Detail label="RFC" value={activeProfile.rfc} /><Detail label="Personalidad" value={detail.legalPersonality.description} /><Detail label="Estatus" value={detail.status.description} /></dl>{message && <div className="notice notice-error" role="alert">{message}</div>}{editing && profile && <form onSubmit={(event) => void save(event)}><p>Los campos de personalidad, RFC y estatus son de sólo lectura.</p>{profile.legalPersonalityCode < 20 ? <><label>Nombre<input value={form.firstName ?? ''} onChange={event => setForm({ ...form, firstName: event.target.value })} required /></label><label>Apellido paterno<input value={form.paternalSurname ?? ''} onChange={event => setForm({ ...form, paternalSurname: event.target.value })} /></label><label>Apellido materno<input value={form.maternalSurname ?? ''} onChange={event => setForm({ ...form, maternalSurname: event.target.value })} /></label><label>Fecha de nacimiento<input type="date" value={form.birthDate ?? ''} onChange={event => setForm({ ...form, birthDate: event.target.value })} required /></label></> : <><label>Razón social<input value={form.legalName ?? ''} onChange={event => setForm({ ...form, legalName: event.target.value })} required /></label><label>Nombre del contacto<input value={form.contactName ?? ''} onChange={event => setForm({ ...form, contactName: event.target.value })} /></label><label>Puesto del contacto<input value={form.contactPosition ?? ''} onChange={event => setForm({ ...form, contactPosition: event.target.value })} /></label></>}<fieldset><legend>Roles</legend>{profile.roles.map(role => <label key={role.roleCode}><input type="checkbox" checked={form.roleCodes?.includes(role.roleCode) ?? false} disabled={!role.isEditable} onChange={event => setForm({ ...form, roleCodes: event.target.checked ? [...(form.roleCodes ?? []), role.roleCode] : (form.roleCodes ?? []).filter(code => code !== role.roleCode) })} /> {role.roleName ?? 'No disponible'}{!role.isActive && ' (No disponible)'}</label>)}</fieldset>{pepRequired && <label><input type="checkbox" checked={pepConfirmed} onChange={event => setPepConfirmed(event.target.checked)} /> Confirmo la revisión PEP.</label>}<div className="side-panel-actions"><button type="button" onClick={() => { setEditing(false); setProfile(null) }} disabled={busy}>Cancelar</button><button type="submit" disabled={busy}>{busy ? 'Guardando…' : 'Guardar datos generales'}</button></div></form>}</section>
}
